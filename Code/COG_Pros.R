...
file.cr = read.csv("C:/Users/surendra_dattatrey/Desktop/S&P Data/Glassdoor/final_combined_review.csv")

#file.cr = file.cr[!is.na(file.cr$text)|file.cr$text != '',]

n = nrow(file.cr)

data = data.frame(id = 1:n, text1 = file.cr$Pros[1:n],
                  stringsAsFactors = F)
data$text = paste(data$text1)
...
#Applyy RowSum and ColSum on DTM
...
#####Calculate ETA - each document's score on each topic
t = Sys.time()
...
###########################################################
#Wordcloud and COG - For each Topic
