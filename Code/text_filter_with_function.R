...
filter_data <- subset(file.spglobal, file.spglobal$Advice.to.Sr..Management == 5)  # 5
#filter_data <- file.spglobal

fun_token <- function ( comment_section, min_tokenval, max_tokenval)
{
  
  data = data.frame(#id = 1:length(filter_data$Text.Response),  # creating doc IDs if name is not given
    text = comment_section ,  # filter_data$Pros
    stringsAsFactors = F)
  ...
  build_dtm_tcm <- function(x){   # x is cleaned corpus
    ...
    vocab = create_vocabulary(it_0,    #  func collects unique terms & corresponding statistics
                              ngram = c(min_tokenval, max_tokenval))
  ...
  return(pros_freq_data_bigrams)
  
}

four_gram_data <- fun_token (filter_data$Pros, 4L, 4L)
tri_gram_data <- fun_token (filter_data$Pros, 3L, 3L)
bi_gram_data <- fun_token (filter_data$Pros, 2L, 2L)

ngram_data <- rbind(tri_gram_data,four_gram_data)
ngram_data
